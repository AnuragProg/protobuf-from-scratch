...
func TestDecodeProjectTypeWithRandomlySpreadTags(t *testing.T) {
	expected := types.ProjectType{
		Name: "John Doe",
		Description: "lorum ipsum",
		Timestamp: 302939020390,
	...
	// insering 0th tag here (for random data check)
	// format tag(fieldno+wiretype)-len-value
	dataBin = append(dataBin, tagsTag)
	dataBin = append(dataBin, byte(len(expected.Tags[0])))
	dataBin = append(dataBin, []byte(expected.Tags[0])...)
	...
	// insering 1st tag here after timestamp (for random data check) 
	// format tag(fieldno+wiretype)-len-value
	dataBin = append(dataBin, tagsTag)
	dataBin = append(dataBin, byte(len(expected.Tags[1])))
	dataBin = append(dataBin, []byte(expected.Tags[1])...)
